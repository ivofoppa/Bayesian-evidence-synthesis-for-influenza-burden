...
dataset <- data.frame(read.csv('ivo_burden_20190523.csv'))
...
### Changing vars into simple values
for (k in 9:length(dataset[1,])) {
  dataset[,k] <- as.integer(sapply(as.vector(dataset[,k]), function(x) ifelse(is.na(x),0,x)))
}

DateHosp <- as.Date(dataset$DateHosp,"%d%B%Y")
...
seaslabls <- sapply(1:nseas, function(s) paste0(10 + s-1,10 + s))
...
## recoding states
statesrev <- sapply(dataset$State, function(s) substr(s,1,2))
dataset$State <- statesrev
## restrictung to given states
selind <- which(dataset$State%in%c("CA","GA","MN","OR","UT","CO","NM","NY","CT","TN","MD","MI"))
dataset <- dataset[selind,]
...
#########################################################################################
setwd(paste0(bfolder,'BEdata'))
outfname <- 'FluSURV-NET-Nation-8seas.Rdata'
