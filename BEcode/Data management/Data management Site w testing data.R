...
for (st in statels) {
  for (seas in 1:nseas) {
    for (ag in 1:5) {
      for (d in 0:1) {
        selind <- which(FSNdata$season==seas & FSNdata$agcat==ag & FSNdata$died==d & substr(FSNdata$state,1,2)==st)
...
for(seas in 1:nseas) {
  ...
  seasselind2 <- which(TestdataCum$season==seas)
  TestseldataCum <- TestdataCum[seasselind2,]

  statels1 <- unique(as.vector(FSNseldata$state))
  statels1 <- unique(sapply(statels1,function(st) substr(st,1,2)))
  
  statels2  <- unique(as.vector(TestseldataCum$state))
...
for (seas in 1:nseas) {
  ...
  for (st in statels) {
    for (ag in 1:5) {
      ...
      for (d in 0:1) {
        Testselind <- which(TestdataCum$state==st & TestdataCum$season==seas & TestdataCum$agcat==ag & 
                              TestdataCum$died==d)
        FSNselind <- which(FSNdataCum$state==st & FSNdataCum$season==seas & FSNdataCum$agcat==ag & 
                             FSNdataCum$died==d)
        ...
        if (length(Testselind)>0 & length(FSNselind)>0){
          row <- c(st,seas,ag,d,tdat,FSNoutcome,Fpop)
...
colnames(Fulldata) <- c("state","season","agcat","died","ntest1","ntest2","ntest3","ntest0","testres1",
...
### Reading-in OSH data      ############################################################
#########################################################################################
OSHfname <- "mort2010-17 osh state.csv"
...
colnames(OSHdata) <- c("season","state","agcat","osh","rcu","pi")
...
setwd(paste0(bfolder,'BEdata'))
### Load data 
outfname <- 'FluSURV-NET-site-osh-7seas.RData'

save(Fulldata,OSHdata,nseas,cipcrlist,sensdata,USpopdata,stateList, file = outfname)
