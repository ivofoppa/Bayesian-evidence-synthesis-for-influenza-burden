...
pfluinit <- c(testpos[,1]/nttype[,1]/sens1init,testpos[,2]/nttype[,2]/exp(logsens2init),testpos[,3]/nttype[,3]/sens3init,(testpos[,1]/sens1init + testpos[,2]/exp(logsens2init) + testpos[,3]/sens3init)/rowSums(nttype[,1:3]))
...
codalsdiff <- data.frame(codals)

setwd(paste0(bfolder,'BEwriteup'))
save(codalsdiff,file = 'codalsdiff.RData')
#########################################################################################
###  ####################################################################################
#########################################################################################
