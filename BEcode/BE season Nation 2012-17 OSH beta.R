...
model.file <- 'BE season OSH beta.txt'
...
outfile <- "codafileFSN_OSH_beta_2012-17.RData"
...
for (ag in 1:5) {
  ...
  FSNfluhospls <- sapply(1:nseas, function(s) {
    selind <- which(Fulldatasel$died==0 & Fulldatasel$season==s)
    ifelse(length(selind)>0,as.numeric(Fulldatasel$Noutcome[selind]))
  })
  
  FSNfludeathls <-  sapply(1:nseas, function(s) {
    selind <- which(Fulldatasel$died==1 & 
            Fulldatasel$season==s)
    
    ifelse(length(selind)>0,as.numeric(Fulldatasel$Noutcome[selind]),0)
    })
  ...
  ptestarr10init <- (nttypearr[,1,1:nseas] + 1)/ptesttot
  ptestarr20init <- (nttypearr[,2,1:nseas] + 1)/ptesttot
  ptestarr30init <- (nttypearr[,3,1:nseas] + 1)/ptesttot
  ptestarr40init <- (nttypearr[,4,1:nseas] + 1)/ptesttot
...
outtablename1 <- "BES hosp beta 2012-17.txt"
outtablename2 <- "BES mort beta 2012-17.txt"
#########################################################################################
#########################################################################################
fromyr <- 2012
toyr <- 2017
