...
###############################################################################
rm(list = ls())
bfolder <- "C:/Users/VOR1/OneDrive/Work related/Misc work/BES and related/Bayesian-evidence-synthesis-for-influenza-burden/" ## project folder
...
###################################################################################################
# fname <- paste0('codaarr no factor ',qual1," ",niter,' National allcause ',fromyr,'-',substr(toyr,3,4),'.RData')
###################################################################################################
###################################################################################################
variables <- c(sapply(1:nseas, function(seas) paste0('EM',seas,'tot')))

agdata <- data.frame(datarr[which(datarr$agcat==3),])
...
for (ag in 1:3){
  ...
  #######################################################################################
  #######################################################################################
  fact <- 1 ## Flu indicator multiplier
...
###################################################################################################
###################################################################################################
setwd(paste0(bfolder,"BEwriteup/BEmanuscript/BEtables"))
# load(fname)
