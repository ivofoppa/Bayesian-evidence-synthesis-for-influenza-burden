...
// [[Rcpp::export]]
List abscSampler(List & megaList, DataFrame & data,int size,double rtio,double crit) {
  List superList = as<List>(megaList["superList"]),inList,outList ;
  DataFrame df ;
  NumericVector parms = as<NumericVector>(megaList["parms"]),absc =  df[0],l = df[1],lvec,abscpart ;
  int maxind = inList["maxind"],zind = inList["zind"],acc,selind ;
  double mx = max(l),zval,uran,psmp,lh,uh,lval ; 
  
  for (int parmind = 1 ; parmind < 3 ; parmind++) {
  ...
  outList = List::create(Named("parms") = parms,Named("superList") = superList) ;
  
  return outList ;
}
/*
// [[Rcpp::export]]
DataFrame sampleLoop(List & megaList, DataFrame & data,int size,double rtio,double crit,int nsims) {
  List superList = as<List>(megaList["superList"]),inList,outList ;
  NumericVector parms = as<NumericVector>(megaList["parms"]) ;
  DataFrame df,parmSamples ;
  NumericVector parmsNew,parm1,parm2 ;

  for (int sim = 1 ; sim < nsims ; sim++) {
    megaList = abscSampler(megaList, data,size,rtio,crit) ;
    parms = megaList["parms"] ;
    parm1.push_back(parms[0]) ;
    parm2.push_back(parms[1]) ;
  }
  parmSamples = DataFrame::create(Named("b0") = parm1,Named("b1") = parm2) ;
  return parmSamples ;
}
  */
