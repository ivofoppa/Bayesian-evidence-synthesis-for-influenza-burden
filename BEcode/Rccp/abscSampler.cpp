...
List abscGroome(List & outList, DataFrame data, int parmind, int size, double rtio) {
  ...
  NumericVector parms = outList["parms"], absc = df[0], l, zabsc, ldiffvec1(0), ldiffvec2(0), abscadd, abscpart ;
  ...
  for (int k = 0; k < absc.size() ; k++) {
    l.push_back(lik(absc[k],parmind,parms,data)) ;
  }
...
NumericVector fliksum(DataFrame & df, int parmind, NumericVector & parms, DataFrame & data, double zval, int maxind, int zind) {
...
double fpsample(DataFrame df, int parmind, NumericVector & parms, DataFrame & data, double zval, 
  ...
  l.insert(selindz,lik(zval, parmind, parms, data)) ;
...
// [[Rcpp::export]]
DataFrame abscSampler(List & superList, DataFrame & data, int size, double rtio, double crit, int nsims) {
  List inList;
  DataFrame parmSampleArr, df ;
  NumericVector absc, l, lvec, parms, abscpart, parmSample, parmS1, parmS2 ;
  ...
  for (int sim = 0 ; sim < nsims ; sim++) {
    ...
    for (parmind = 1 ; parmind <3 ; parmind++) {
      ...
      inList = abscGroome(inList,data,parmind,size,rtio) ;
       ...
       uran = R::runif(0,1) ;
        ...
        if (uran < lh/uh) {
          ...
          if (uran < lval/uh) {
            ...
            l.insert(selind,lik(psmp,parmind,parms,data)) ;
            mx = max(l) ;
            df = DataFrame::create(Named("absc") = absc,Named("l") = l) ;
            ...
            inList = List::create(Named("df") = df, Named("parmind") = parmind, Named("parms") = parms, Named("zval") = zval, Named("maxind") = maxind, Named("zind") = zind) ;
