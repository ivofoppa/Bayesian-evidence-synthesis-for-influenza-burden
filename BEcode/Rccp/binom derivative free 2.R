...
### Upper hull
fupperhull <- function(p,absc,f) {
  selind <- tail(which(absc <= p),1)
  xcap <- p - absc[selind]
  
  if (selind%in%lowerind) {
    fexp <- exp(f[selind])
    fval <- fexp
    
  } else if (selind%in%middleind) {
    if (p <= zval) {
      f0 <- f[selind - 1]
      f1 <- f[selind]
      p0 <- absc[selind - 1]
      p1 <- absc[selind]
      a <- (f1 - f0)/(p1 - p0)
      fval <- exp(f1)/a*(exp(a*(zval - p1)) - exp(a*(p - p1)))
    } else if (p > zval) {
      f0 <- f[selind]
      f1 <- f[selind + 1]
      p0 <- absc[selind]
      p1 <- absc[selind + 1]
      a <- (f1 - f0)/(p1 - p0)
      fval <- exp(f0)/a*(exp(a*(zval - p0)) - exp(a*(p - p1)))
