...
List abscPrep(NumericVector & absc0,NumericVector & parms,int parmind,DataFrame & data,double rtio) {
  List outList ;
  DataFrame df ;
  ...
  outList = List::create(Named("df") = df,Named("parmind") = parmind,Named("zval") = zval,Named("maxind") = maxind,Named("zind") = zind) ;
...
List abscGroome(List & outList,NumericVector parms,int parmind,DataFrame data,int size,double rtio) {
  ...
  zval = intsct(df,parms,parmind,data,maxind) ;
  zind = fzind(df,parms,parmind,data,zval) ;
  List TotList = List::create(Named("df") = df,Named("parms") = parms,Named("parmind") = parmind,Named("zval") = zval,Named("maxind") = maxind,Named("zind") = zind) ;
  
  return TotList ;
}

// [[Rcpp::export]]
List TotPrep(NumericVector & absc,NumericVector & parms,int parmind,DataFrame & data,double rtio) {
  List TotList ;
  
  TotList = abscPrep(absc,parms,parmind,data,rtio) ;
