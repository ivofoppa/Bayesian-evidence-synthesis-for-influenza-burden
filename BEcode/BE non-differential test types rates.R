...
data <- list('nfluhospobs'=nfluhospobs,'FSNpop'=FSNpop,'Npop'=Npop,
             ...
             'nttype'=nttype,'testpos'=testpos,'pcrsens'=pcrsens,'ntot'=ntot)
...
pfluinit <- sum(testpos)/sum(nttype[,1:3])

fluposinit <- round(pfluinit*nttype[,1:3])
for (k in 1:2){
  for (i in 1:3){
    if (fluposinit[k,i] <= testpos[k,i]){
      fluposinit[k,i] <- testpos[k,i]+1
    }
  }
}
...
niter <- 10000
nadapt <- 10000
setwd(paste0(bfolder,'git project'))
