...
double fbin(double p, NumericVector parms){
  ...
  double f = fdist(p,parms,dist) ;
  return f ;
} 

double fpoi(double r, NumericVector parms){
  double x = parms[0] ;
  double f = R::dpois(x,r,parms,dist) ;
  return f ;
} 
    ...
    if  (maxind==2 ) {
      if (fmxind < 3) {
        while (fmxind < 3) {
          ...
          newabsc = minp + abscinc ;
          absc.insert(1, newabsc) ;
          f.insert(1,R::dpois(x,newabsc,parms,dist)) ;
          fmxind = which_max(f) ;
        }
      } 
      if (fmxind>=(absc.size() - 3) || fabscmin > -50) {
        while (fmxind>=(absc.size() - 3) || fabscmin > -50) {
          ...
          f.push_back(R::dpois(x,newabsc,parms,dist)) ;
          fmxind = which_max(f) ;
          fabscmin = f[f.size() - 1] ;
        }
      }
...
int maxindC(DataFrame & df, NumericVector & parms, std::string & dist ){
  ...
  NumericVector absc = df[0], f = df[1] ;
    ...
    if (f[fmxind] > R::dpois(x,arg1,parms,dist) && f[fmxind] > R::dpois(x,arg2,parms,dist)) {
      maxind = 0 ;
    } else if (f[fmxind] < R::dpois(x,arg2,parms,dist)) {
      maxind = 1 ;
    } else if (R::dpois(x,arg1,parms,dist) > f[fmxind]) {
      maxind = 2 ;
    }
  }
  return maxind ;
...
double intsct(NumericVector zabsc, NumericVector & parms, std::string & dist){
  ...
  if (dist=="binom") {
    ...
    f1 = R::dpois(x,p1,parms,dist) ;
    f2 = R::dpois(x,p2,parms,dist) ;
    f3 = R::dpois(x,p3,parms,dist) ; 
    f4 = R::dpois(x,p4,parms,dist) ;
...
DataFrame fabscaug(DataFrame df, NumericVector & parms, std::string dist, double zval) {
  ...
  if (dist=="binom") {  
    ...
    int x = parms[0] ;
    f.insert(selind,R::dpois(x,zval,parms,dist)) ;
  }
...
int fzind(DataFrame df, NumericVector & parms, std::string dist, double zval) {
  ...
  if (dist=="binom") {  
    ...
    if (R::dpois(x,arg,parms,dist) > R::dpois(x,zval,parms,dist)) {
      zind = 1 ;
    } else {
      zind = 0 ;
    }
