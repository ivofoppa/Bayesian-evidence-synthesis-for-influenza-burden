...
for (agcat in 1:5){
  ...
  pt20init <- sapply(nttype[,2]/ntot, function(x) ifelse(x > .5,min(x,.95),max(x,.1)))
  pt30init <- sapply(nttype[,3]/ntot, function(x) ifelse(x > .5,min(x,.95),max(x,.1)))
  pt40init <- sapply(nttype[,4]/ntot, function(x) ifelse(x > .5,min(x,.95),max(x,.1)))
